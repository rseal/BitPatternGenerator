...
namespace arecibo{
   ...
   {
      for(unsigned int idx=0; idx<tv.size(); ++idx)
      {
         phaseKey.Process(tv[idx]);
         bwKey.Process(tv[idx]);
         gKey.Process(tv[idx]);
         saKey.Process(tv[idx]);
         t1Key.Process(tv[idx]);
         t2Key.Process(tv[idx]);
         t3Key.Process(tv[idx]);
      }
