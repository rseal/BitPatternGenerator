...
namespace lcd{
  typedef boost::tuple<std::string, uint8_t> Tuple;
};
...
class LCD {
  ...
  virtual void Location(const uint8_t line, const uint8_t column)=0;
  virtual void Write(const std::string& str)=0;
  virtual void CommInit()=0;
  virtual void Home()      =0;
  virtual void Control(const uint8_t num)=0;
...
public:
  ...
  LCD(const uint8_t line, const uint8_t columns):
  ...
  virtual void Brightness(const uint8_t num){}
  virtual void Contrast(const uint8_t num){}
  ...
  void Message(const uint8_t line, const std::string str){
...
void LCD::Update(){
  ...
  uint8_t lines = newMessage_.size();
  uint8_t vecSize;

  for(uint8_t i=0; i<lines; ++i){
    vec = Compare(newMessage_[i],oldMessage_[i]);
    vecSize = vec.size();
    for(uint8_t j=0; j<vecSize; ++j){
...
const std::vector<lcd::Tuple> LCD::Compare(std::string& newStr, std::string& oldStr){
  uint8_t size = newStr.size() < oldStr.size() ? newStr.size() : oldStr.size();
  uint8_t start=0;
  bool mark=false;
  std::vector<lcd::Tuple> vec;
