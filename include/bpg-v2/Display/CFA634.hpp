...
class CFA634: public LCD{
  ...
  void Location(const uint8_t line, const uint8_t column){
  ...
  void Control(const uint8_t num){
...
public:
  ...
  CFA634(const std::string& port, const uint8_t lines, const uint8_t columns): LCD(lines,columns), port_(port) {}
  ...
  void Brightness(const uint8_t num){
    //0-100
    unsigned char buf[2] = {14,num};
    write(fileDescriptor_, &buf,2);
  }

  void Contrast(const uint8_t num){
    //0-100
