...
class ControlStatus: public SThread 
{
   ...
   public:    
      ...
      bool Reset(int channelNumber)
      { 
         return 
            channelNumber == PORTAB ? reset1_ && reset2_ :
            channelNumber == PORTA  ? reset1_ : reset2_;
      }

      /// Starts the loaded mode on the specified PORT
      bool Start(int channelNumber)
      { 
         return 
            channelNumber == PORTAB ? start1_ && start2_ :
            channelNumber == PORTA  ? start1_ : start2_;
      }

      /// Returns the active buffer for the specified port
      bool ActiveBuffer(int channelNumber)
      { 
         return 
            channelNumber == PORTAB ? activeBuffer1_ && activeBuffer2_ :
      ...
      bool Preload(int channelNumber)
      { 
         return 
            channelNumber == PORTAB ? preload1_ && preload2_ :
            channelNumber == PORTA  ? preload1_ : preload2_;
      }

      /// Tells the host PC that the last mode sent is properly loaded
      bool LoadComplete(int channelNumber)
      { 
         return 
            channelNumber == PORTAB ? loadComplete1_ && loadComplete2_ :
      ...
      bool Ready(int channelNumber)
      {
         return 
            channelNumber == PORTAB ? ready1_ && ready2_ :
            channelNumber == PORTA  ? ready1_ : ready2_;
      }

      /// Returns true if the specified port is currently active
      bool Active(int channelNumber)
      {
         return 
            channelNumber == PORTAB ? active1_ && active2_ :
      ...
      int Status(int type, int channelNumber)
      {
         int result;
         ...
         switch(type)
         {
            case RESET: 
               {
                  result = channelNumber ? reset1_ : reset2_; 
                  break;
               }
            case START: 
               {
                  result = channelNumber ? start1_ : start2_; 
                  break;
               }
            case STOP:  
               {
                  result = channelNumber ? stop1_  : stop2_; 
                  break;
               }
            case SYNC:  
               {
      ...
      enum{RESET=0, START=1, STOP=2, SYNC=3, CLK_SOURCE=4, SWITCH=5};
      enum{SET=1, CLEAR=0};
